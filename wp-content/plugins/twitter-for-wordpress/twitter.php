	...
	if ($username == '') {
        ...
        $i = 0;
				foreach ( $messages->items as $message ) {
					...
					if ($list) echo '<li class="twitter-item"><h4>Twitter</h4><p>'; elseif ($num != 1) echo '<p class="twitter-message">';
          ...
          if ($twitter_users)  { $msg = twitter_users($msg); }
            ...
            if($linked == 'all')  { 
              $msg = '</p><p><a href="'.$link.'" class="twitter-link">'.$msg.'</a>';  // Puts a link to the status of each tweet 
            } else {
              $msg = $msg . '</p><p><a href="'.$link.'" class="twitter-link">'.$linked.'</a>'; // Puts a link to the status of each tweet
        ...
        if($update) {				
          ...
          echo sprintf( __('%s', 'twitter-for-wordpress'),' <span class="twitter-timestamp"><abbr title="' . date(__('Y/m/d H:i:s'), $time) . '">' . $h_time . '</abbr></span>' );
					...
					if ($list) echo '<p></li>'; elseif ($num != 1) echo '</p>';
